...
class Strategy(StrategyBase):
    ...
    def global_variables(self):
        ...
        self.ma_exit = show_variable(50, GlobalType.INT)
        self.donchian_in = show_variable(55, GlobalType.INT)
        self.donchian_out = show_variable(20, GlobalType.INT)

        self.atr_window = show_variable(22, GlobalType.INT)
        self.chandelier_k = show_variable(3.0, GlobalType.FLOAT)
        self.target_vol = show_variable(0.15, GlobalType.FLOAT)

        self.pool_budget_pct = show_variable(0.95, GlobalType.FLOAT)
        self.max_concurrent_holdings = show_variable(6, GlobalType.INT)
    ...
    def handle_data(self):
        ...
        for sym in self._pool:
            ...
            cond_long_trend = ma_long_val > 0 and current_price > ma_long_val
            cond_breakout = donchian_high > 0 and current_price >= donchian_high
            cond_concurrent = held_count < int(self.max_concurrent_holdings)

            if cond_regime and cond_long_trend and cond_breakout and cond_concurrent:
                score = 0.0
