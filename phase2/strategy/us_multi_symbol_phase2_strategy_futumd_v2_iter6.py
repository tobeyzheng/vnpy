...
class Strategy(StrategyBase):
    ...
    def global_variables(self):
        # ---- iter2: tighter pool concentration + entry grace period ----
        self.ma_long = show_variable(100, GlobalType.INT)
        self.ma_regime = show_variable(200, GlobalType.INT)
        self.ma_exit = show_variable(100, GlobalType.INT)
        # Donchian shortened further to 10 — entry on early breakouts.
        self.donchian_in = show_variable(10, GlobalType.INT)
        self.donchian_out = show_variable(10, GlobalType.INT)

        self.atr_window = show_variable(40, GlobalType.INT)
        # Chandelier multiplier 5 → 6 — give NVDA / TSLA more rope.
        self.chandelier_k = show_variable(6.0, GlobalType.FLOAT)
        self.target_vol = show_variable(0.25, GlobalType.FLOAT)

        self.pool_budget_pct = show_variable(0.99, GlobalType.FLOAT)
        # Concentrate: only top-3 winners hold simultaneously, single
        # position size jumps from ~1/8 NAV to ~1/3 NAV.
        self.max_concurrent_holdings = show_variable(3, GlobalType.INT)
        self.cash_buffer_pct = show_variable(0.01, GlobalType.FLOAT)
        self.max_orders_per_day = show_variable(10, GlobalType.INT)
        self.cooldown_bars_after_exit = show_variable(20, GlobalType.INT)

        # iter2: minimum hold period before any exit (except absolute
        # disaster) can fire — stops the 'BUY today, SELL next day'
        # pattern induced by tight ATR rails right after entry.
        self.min_hold_bars = show_variable(10, GlobalType.INT)
        # iter6: tighter single-position disaster guard 12% → 8%.
        self.disaster_loss_pct = show_variable(0.08, GlobalType.FLOAT)
        # iter3: regime-flip flat — if Close < SMA(ma_regime) on a
        # held bar, force-exit regardless of Chandelier.  Empirically
        # cuts 2022 drawdown by ~half at the cost of a few re-entries.
        self.regime_flat_enabled = show_variable(True, GlobalType.BOOL)

        # iter6: portfolio-level high-water-mark drawdown circuit.
        # When NAV drops more than this fraction below its rolling peak,
        # all positions are flattened until next regime re-entry.
        self.portfolio_dd_cut_pct = show_variable(0.15, GlobalType.FLOAT)
        # In-memory tracker (set in initialize / handle_data, never
        # persisted to disk per the no-IO rule).
        self._nav_high_water = 0.0
        self._portfolio_paused = False

        # iter5: pyramid add-on — once a position is +1·ATR above its
        # last entry, allow buying another half-slice; cap total slices
        # at max_slices.  Lets winners (NVDA / AVGO 2023-2024) compound
        # without breaking the per-symbol concentration logic.
    ...
    def handle_data(self):
        ...
        # iter6: portfolio-level high-water-mark drawdown circuit.
        # Update HWM, then if NAV is more than portfolio_dd_cut_pct
        # below it, flatten everything and pause new entries until NAV
        # claws back to within half the threshold of HWM.
        if nav_snapshot > self._nav_high_water:
            self._nav_high_water = nav_snapshot
        cut = float(self.portfolio_dd_cut_pct)
        if cut > 0 and self._nav_high_water > 0 and nav_snapshot > 0:
            dd = 1.0 - nav_snapshot / self._nav_high_water
            if dd >= cut:
                self._portfolio_paused = True
            elif dd <= cut * 0.5:
                self._portfolio_paused = False
        ...
        for sym in self._pool:
            ...
            if held_qty > 0:
                ...
                exit_reason = None
                bars_held = st["bars_since_last_entry"]
                # iter6 priority -2: portfolio drawdown circuit \u2014 once
                # tripped, dump everything immediately.
                if self._portfolio_paused:
                    exit_reason = "portfolio_dd_cut"
                # iter3 priority -1: regime flat \u2014 if SMA(200) flips
                # bearish on this bar, dump everything regardless of
                # min_hold or ATR rail.
                if (exit_reason is None
                        and bool(self.regime_flat_enabled)
                        and ma_regime_val > 0
                        and current_price < ma_regime_val):
                    exit_reason = "regime_flat"                # iter2 priority 0: absolute disaster cut — always armed,
                # even within the min_hold_bars grace period.
                if exit_reason is None and st["entry_price"] > 0 and float(self.disaster_loss_pct) > 0:
                    loss = (current_price / st["entry_price"]) - 1.0
                    if loss <= -float(self.disaster_loss_pct):
                        exit_reason = "disaster_stop"
                # iter2: skip slow exits during the grace period so a
                # single 1-day post-entry pullback can't wash us out.
                if exit_reason is None and bars_held >= int(self.min_hold_bars):
                ...
                # iter5: pyramid eligibility check \u2014 already-held position
                # may still queue an add-on if price has advanced 1 ATR
                # from the last fill and we still have slices to spend.
                slices_used = int(st.get("used_slices", 0) or 0)
                slices_max = int(self.max_slices)
                last_entry = float(st.get("last_entry_price", 0.0) or 0.0)
                step = float(self.pyramid_atr_step)
                if (atr_val > 0 and last_entry > 0
                        and slices_used < slices_max
                        and bars_held >= int(self.min_hold_bars)
                        and current_price >= last_entry + step * atr_val):
                    candidates.append({
                        "symbol": sym,
                        "price": current_price,
                        # Pyramid candidates score lower than fresh
                        # entries so cash goes to new winners first.
                        "score": -1.0,
                        "atr": atr_val,
                        "reasons": ["Pyramid+1ATR"],
                    })
                # Held positions never go through the fresh-entry path.
                continue
            ...
            # iter6: portfolio drawdown circuit overrides entry attempts.
            cond_not_paused = not self._portfolio_paused

            if cond_regime and cond_breakout and cond_concurrent and cond_not_paused:
                score = 0.0
    ...
    def _enter_position(self, symbol, price, reasons):
        ...
        # iter5: pyramid add-on path — active when we already hold and
        # price has moved up by at least pyramid_atr_step × ATR from
        # the last fill, and we have not used all max_slices yet.
        is_pyramid = False
        if held_qty > 0:
            atr_val0 = float(st.get("last_atr", 0.0) or 0.0)
            last_entry = float(st.get("last_entry_price", 0.0) or 0.0)
            slices_used = int(st.get("used_slices", 0) or 0)
            slices_max = int(self.max_slices)
            step = float(self.pyramid_atr_step)
        ...
        # Pyramid slices use a fraction of the base symbol budget so
        # later adds are smaller than the initial entry (Turtle-style).
        if is_pyramid:
            per_symbol_budget = per_symbol_budget * float(self.pyramid_size_pct)
        avail_cash = float(cash(currency=Currency.USD) or 0.0)
        ...
        tag = "加仓" if is_pyramid else "开仓"
        msg = "BUY {sym} qty={q} price={p:.2f} (v2 trend {tag}) 原因:{r}".format(
            sym=symbol, q=qty, p=buy_price, r=reason_str, tag=tag,
        )
        ...
        if self.LIVE_SUBMIT:
            ...
            st["used_slices"] = (st.get("used_slices", 0) or 0) + 1
        ...
        st["last_signal"] = "{t}(v2) {r}".format(t=tag, r=reason_str)
