...
GATEWAY_NAME = "FUTU"
STRATEGY_CLASS = "SoxlLlmCtaStrategy"
STRATEGY_NAME = "soxl_llm_cta"
...
STRATEGY_SETTING = {
    ...
    "llm_signal_dir": "examples/futu_trader/output/llm_signals",
    "llm_symbol": VT_SYMBOL,
    "llm_min_confidence": 0.55,
    "llm_stale_after_hours": 30,
    "llm_high_risk_threshold": 0.75,
    "llm_block_risk_threshold": 0.85,
...
def main() -> None:
    ...
    if STRATEGY_CLASS not in cta_engine.classes:
        logger.error(f"找不到策略类：{STRATEGY_CLASS}，请检查 strategies/soxl_llm_cta_strategy.py")
        main_engine.close()
        sys.exit(1)
    ...
    logger.info(f"LLM增强策略已启动：{STRATEGY_NAME}")
