...
def main() -> None:
    ...
    store = SimAccountStore(repo / 'state' / 'runs' / 'us_sim_account.json')
    account = store.load()
    ...
    quote_map = {item['code'].replace('US.', '') + '.US': float(item['price']) for item in snapshot.get('items', []) if item.get('price') is not None}
    ...
    out = repo / 'state' / 'runs' / 'us_sim_close_report.json'
    drawdown = 0.0
