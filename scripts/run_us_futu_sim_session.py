...
FALLBACK_SYMBOLS = ["MSFT.US", "NVDA.US", "AVGO.US", "AMD.US", "AAPL.US", "GOOGL.US", "AMZN.US", "META.US"]
OPEN_ORDER_STATUSES = {"SUBMITTED", "SUBMITTING", "WAITING", "WAITING_SUBMIT", "SUBMITTED_ALL"}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="美股富途模拟盘会话；按预算/亏损约束运行到美股收盘并生成报告")
    parser.add_argument("--max-budget", type=float, default=100000.0)
    parser.add_argument("--max-order-value", type=float, default=10000.0)
...
class UsFutuSimSession:
    ...
    def run(self) -> dict[str, Any]:
        ...
        while True:
            ...
            try:
                ...
                elif self._is_regular_session(now_et):
                    self._evaluate_and_trade(snapshot)
                else:
                    self.actions.append({"time": datetime.now().isoformat(timespec="seconds"), "action": "wait", "reason": "outside_regular_session", "et": now_et.isoformat()})
    ...
    def _snapshot(self) -> dict[str, Any]:
        ...
        symbols = sorted({self._to_vt_symbol(p.get("code") or p.get("symbol", "")) for p in self._us_positions(positions.get("items", [])) if p.get("qty", 0)})
    ...
    def _evaluate_and_trade(self, snapshot: dict[str, Any]) -> None:
        ...
        for candidate in candidates:
            ...
            if not quote:
                continue
            evaluation = self.strategy_engine.evaluate_candidate(candidate=candidate, quote=quote, flow_divisor=5e9, has_event_catalyst=self._has_catalyst(candidate))
    ...
    def _reduce_losing_positions(self, snapshot: dict[str, Any]) -> None:
        for pos in self._us_positions(snapshot.get("positions", {}).get("items", [])):
    ...
    def _submit(self, symbol: str, side: str, qty: int, price: float, reason: str) -> None:
        request_id = f"us_futu_sim_{datetime.now().strftime('%Y%m%d%H%M%S')}_{symbol.replace('.', '_')}_{side}"
        try:
    ...
    def _record_order_state(self, payload: dict[str, Any], reason: str) -> None:
        ...
        intent = OrderIntent(
            ...
            market="us",
            strategy_id="us_futu_sim_session",
    ...
    def _candidate_rows(self, snapshot: dict[str, Any]) -> list[dict[str, Any]]:
        rows = UnifiedCandidateProvider(REPO_ROOT).load("us")[: self.args.max_symbols]
        existing = [self._to_vt_symbol(p.get("code") or p.get("symbol", "")) for p in self._us_positions(snapshot.get("positions", {}).get("items", [])) if p.get("qty", 0)]
        seen = {row.get("symbol") for row in rows}
        for symbol in [*existing, *FALLBACK_SYMBOLS]:
            if symbol and symbol not in seen:
                rows.append({"symbol": symbol, "market": "us", "name": symbol, "raw_score": 0.62, "rationale": "existing_or_fallback_us_candidate"})
                seen.add(symbol)
        return rows[: self.args.max_symbols]

    def _write_report(self, *, snapshot: dict[str, Any], final: bool) -> dict[str, Any]:
        report = {
            "task": "us_futu_sim_session",
    ...
    def _equity(self, snapshot: dict[str, Any]) -> float:
        ...
        positions_value = sum(float(p.get("nominal_price") or 0) * int(p.get("qty") or 0) for p in self._us_positions(snapshot.get("positions", {}).get("items", [])))
        return cash + positions_value

    def _used_budget(self, snapshot: dict[str, Any]) -> float:
        return sum(max(float(p.get("nominal_price") or p.get("cost_price") or 0), 0.0) * max(int(p.get("qty") or 0), 0) for p in self._us_positions(snapshot.get("positions", {}).get("items", [])))

    def _current_loss(self, snapshot: dict[str, Any]) -> float:
        unrealized_loss = sum(max((float(p.get("cost_price") or 0) - float(p.get("nominal_price") or 0)) * int(p.get("qty") or 0), 0.0) for p in self._us_positions(snapshot.get("positions", {}).get("items", [])))
    ...
    def _held_qty(self, snapshot: dict[str, Any], symbol: str) -> int:
        for p in self._us_positions(snapshot.get("positions", {}).get("items", [])):
            if self._to_vt_symbol(p.get("code") or p.get("symbol", "")) == symbol:
                return int(p.get("qty") or 0)
        return 0

    def _us_positions(self, rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        return [row for row in rows if self._to_vt_symbol(row.get("code") or row.get("symbol", "")).endswith(".US")]

    def _has_catalyst(self, candidate: dict[str, Any]) -> bool:
        text = str(candidate.get("rationale") or "")
        return any(word in text for word in ["AI", "catalyst", "催化", "earnings", "突破"])
...
def main() -> None:
    ...
    report = UsFutuSimSession(args).run()
    print(json.dumps({"report": str(REPO_ROOT / "state" / "runs" / "us_futu_sim_session_report.json"), "final": report.get("final"), "orders": len(report.get("orders", []))}, ensure_ascii=False))
