...
- **`candidate_framework`**
  - 从候选输入中筛出观察标
  - `preferred_market` 在 workflow 内会先做别名归一化，当前会把 `hk` / `hongkong` 统一映射到 `hong_kong`
...
- **`backtest`**
  ...
  - 对港股 `hong_kong` 候选，只要当前 bucket 仍属于活跃候选集合（如 `priority_trade` / `active_watch` / `research_queue`）、不存在 `hard_risk_flags`，且 cadence 已被规则层判定为 `daily` 或 `minute`，就允许先进入 `backtest` 证据整理 / 执行集合；是否升级到更高阶段仍由 readiness 与后续门禁决定
- **`readiness`**
